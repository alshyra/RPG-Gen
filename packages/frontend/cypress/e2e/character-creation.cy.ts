describe('Character creation single flow', () => {
  ...
  it('should allow user to create a character', () => {
    // start creation
    cy.contains('button', 'Créer un personnage').click();
    ...
    cy.contains('div', 'Humain').click();
    cy.contains('button', 'Suivant').should('not.be.disabled').click();

    // class selection + ability score interactions
    cy.get('select').select('Bard').should('have.value', 'Bard');
    cy.get('[data-test-id="ability-score-Cha"]').should('exist');
    ...
    cy.get('[data-test-id="ability-score-Str"]')
      ...
      .click()
      .click();
    cy.get('[data-test-id=ability-score-Str] [data-test-id="ability-score"]').should(
      'contain',
      '8',
    );
    cy.get('[data-test-id="ability-score-Dex"]').contains('+').click();
    cy.get('[data-test-id=ability-score-Dex] [data-test-id="ability-score"]').should(
      'contain',
      '15',
    );

    cy.get('[data-test-id="ability-score-Int"]').contains('-').click().click();
    cy.get('[data-test-id=ability-score-Int] [data-test-id="ability-score"]').should(
      'contain',
      '10',
    );
    cy.get('[data-test-id="ability-score-Cha"]')
      ...
      .click()
      .click();
    cy.get('[data-test-id=ability-score-Cha] [data-test-id="ability-score"]').should(
      'contain',
      '15',
    );
    ...
    cy.get('[data-testid="ui-checkbox"][for="skill-Persuasion"]').click();
    cy.get('[data-testid="ui-checkbox"][for="skill-Stealth"]').click();
    cy.get('[data-testid="ui-checkbox"][for="skill-Performance"]').click();
    ...
    cy.contains('Main de mage')
      ...
      .check({ force: true });
    cy.contains('Message').parent().parent().find('input[type="checkbox"]').check({ force: true });
    ...
    cy.contains('Mot de guérison')
      ...
      .find('input[type="checkbox"]')
      .check({ force: true });
    cy.contains('Soins').parent().parent().find('input[type="checkbox"]').check({ force: true });
    cy.contains('Murmures dissonants')
      .parent()
      .parent()
      .find('input[type="checkbox"]')
      .check({ force: true });
    cy.contains('Sommeil').parent().parent().find('input[type="checkbox"]').check({ force: true });
    ...
    cy.contains('button', 'Terminer').should('not.be.disabled').click();
