...
describe('Character Creation', () => {
  ...
  it('should navigate through all character creation steps', () => {
    ...
    cy.contains('Dungeons & Dragons').closest('.tpl')
      .find('button')
      .contains('Commencer')
      .click();
    ...
    cy.get('input[placeholder="Ex: Aragorn"]').clear()
      .type('TestHero');

    cy.contains('♂️ Homme').click();
    cy.contains('Humain').click();
    cy.contains('button', 'Suivant').click();
    cy.contains('Sélection de la classe').should('be.visible');
    ...
    cy.get('input[type="checkbox"]').then(($cb) => {
      if ($cb.length > 0) {
        cy.wrap($cb).first()
          .scrollIntoView()
          .check({ force: true });
        if ($cb.length > 1) cy.wrap($cb).eq(1)
          .scrollIntoView()
          .check({ force: true });
      }
    });
    ...
    cy.contains('Sorts').should('be.visible');
    cy.get('[data-testid="ui-checkbox"]').first()
      .scrollIntoView()
      .click({ force: true });
    cy.contains('button', 'Suivant').click();
  ...
  it('should save character draft to the server (draft persisted)', () => {
    ...
    cy.wait('@updateCharacter').then((interception) => {
      ...
      if (interception.request.body?.name !== 'DraftHero') {
        cy.wait('@updateCharacter').its('request.body')
  ...
  it('should restore draft on page refresh', () => {
    ...
    cy.wait('@updateCharacter2').then((interception) => {
      if (interception.request.body?.name !== 'RefreshHero') {
        cy.wait('@updateCharacter2').its('request.body')
  ...
  it('should persist ability scores (character.scores) on page refresh', () => {
    ...
    cy.get('[data-test-id="ability-score-Con"]').should('exist');

    // Spy on the save character request so we can ensure the change persisted.
    cy.intercept('PUT', '**/api/characters/*').as('updateCharacter');
    cy.get('[data-test-id="ability-score-Str"]').contains('-')
      .click();
    cy.get('[data-test-id="ability-score-Dex"]').contains('+')
      .click();
  ...
  it('should auto-generate and save avatar when finishing character creation', () => {
    ...
    cy.contains('Scores de capacités').should('be.visible');
    cy.get('[data-test-id="ability-score-Con"]').contains('+')
      .click();
    cy.contains('button', 'Suivant').should('not.be.disabled');
    cy.contains('button', 'Suivant').click();
    cy.url().should('match', /\/character\/[^/]+\/step\/3/);
