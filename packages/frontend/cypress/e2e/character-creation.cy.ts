...
describe('Character Creation', () => {
  ...
  it("should navigate through all character creation steps", () => {
    ...
    cy.get("select").select("Wizard").should('have.value','Wizard');
    // proceed to skills
    cy.contains("button", "Suivant").click();
    ...
    cy.get('input[type="checkbox"]').then(($cb) => {
      if ($cb.length > 0) {
        cy.wrap($cb).first().scrollIntoView().check({ force: true });
        if ($cb.length > 1) cy.wrap($cb).eq(1).scrollIntoView().check({ force: true });
      }
    });
    cy.contains("Compétences").should("be.visible");
    cy.contains("Compétences").should("be.visible");
  ...
  it("should persist ability scores (character.scores) on page refresh", () => {
    ...
    cy.contains("button", "Suivant").click();
    cy.contains("Sélection de la classe").should("be.visible");
    cy.get("select").select("Fighter").should('have.value','Fighter');
    ...
    cy.url().should('match', /\/character\/([^/]+)\/step\//).then((u) => {
  ...
  it("should auto-generate and save avatar when finishing character creation", () => {
    ...
    cy.get('[data-test-id="ability-score-Con"]').contains("+").click();
    cy.contains("button", "Suivant").should('not.be.disabled');
    cy.contains("button", "Suivant").click();
    cy.url().should("match", /\/character\/[^/]+\/step\/3/);
    // If this class exposes skill choices, choose up to two
