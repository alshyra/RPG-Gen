describe('Home Page', () => {
  beforeEach(() => {
    ...
    cy.prepareE2EDb({ count: 2 }).then(r => {
  ...
  it('should display character list when characters exist', () => {
    ...
    cy.get('body').then($body => {
      ...
      if (text.includes('Aucun personnage trouvé')) {
        cy.contains(/Aucun personnage trouvé/).should('be.visible');
      } else {
        cy.contains('Mes personnages').should('be.visible');
        // Ensure UI includes character cards with aria-label containing "Reprendre"
        cy.get('[role="button"][aria-label*="Reprendre"]').should('have.length.gte', 1);
      }
    });

    // Should have clickable character cards and delete buttons
    cy.get('[role="button"][aria-label*="Reprendre"]').should('exist');
    cy.get('button[aria-label*="Supprimer"]').should('exist');
    // At least one character card should be available for existing characters
    cy.get('[role="button"][aria-label*="Reprendre"]').should('have.length.gte', 1);
    cy.get('button[aria-label*="Supprimer"]').should('have.length.gte', 1);
