describe('Combat flow', () => {
  ...
  it('loads combat panel with visual arena and verifies state', () => {
    ...
    cy.intercept('GET', '**/api/combat/*/status').as('combatStatus');
    ...
    cy.wait('@getCharacters').then(interception => {
      ...
      cy.get('[data-cy="combat-panel"]', { timeout: 10000 }).should('exist');

      cy.get('[data-cy="combat-arena"]', { timeout: 5000 }).should('exist');
