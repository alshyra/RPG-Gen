...
describe('authService', () => {
  ...
  const createMockToken = (exp: number): string => {
    ...
    const payload = btoa(JSON.stringify({ sub: 'user123', email: 'test@example.com', exp }));
  ...
  describe('isTokenExpired', () => {
    ...
    it('should return true for a token without exp claim', () => {
      ...
      expect(authService.isTokenExpired(noExpToken)).toBe(true);
    });
    ...
    it('should return true for a malformed token with wrong number of parts', () => {
      expect(authService.isTokenExpired('part1.part2')).toBe(true);
      expect(authService.isTokenExpired('part1.part2.part3.part4')).toBe(true);
    });
  });
});
