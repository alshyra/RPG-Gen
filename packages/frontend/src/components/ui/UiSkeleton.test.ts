...
describe('UiSkeleton', () => {
  ...
  it('renders with avatar variant', () => {
    ...
    expect(wrapper.find('div').classes()).toContain('h-12');
  ...
  it('renders with card variant', () => {
    ...
    expect(wrapper.find('div').classes()).toContain('h-24');
    expect(wrapper.find('div').classes()).toContain('w-full');
  });
  ...
  it('has correct accessibility attributes', () => {
    ...
    expect(wrapper.find('div').attributes('aria-label')).toBe('Loading');
  });
});
