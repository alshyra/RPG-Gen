...
export class Character {
  ...
  @Prop({ required: true })
  characterId: string; // UUID kept for deterministic client-side IDs and localStorage keys
  ...
  @Prop({ default: false })
  isDeceased: boolean; // archived characters won't be used in active sessions

  @Prop()
  diedAt: Date; // used only if isDeceased === true

  @Prop()
  deathLocation: string; // optional, useful for logs & story traces
}

export const CharacterSchema = SchemaFactory.createForClass(Character);

// Create compound indexes to speed frequent queries:
// - Look up a character by (userId, characterId) when loading a session.
// - Filter by userId + isDeceased for quick list views and archive management.
