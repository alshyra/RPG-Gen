...
export class ActionRecordService {
   ...
   */
  async tryAcquire(actionToken: string, metadata: { requesterId: string; combatId: string; expectedDto: string; idempotencyKey?: string }) {
    ...
    if (!acquired) {
      ...
      if (existing && existing.status === ActionStatus.APPLIED) return { acquired: false, record: existing };
      return { acquired: false, record: existing };
    }

    return { acquired: true, record: acquired };
  }

  async setApplied(actionToken: string, resultPayload: unknown, opts?: { idempotencyKey?: string; requesterId?: string }) {
    const update: any = {
