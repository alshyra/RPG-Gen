<template>
    <div
        ...
        <div class="flex items-center gap-2">
            <!-- Message input -->
            <input v-model="playerText" @keydown.enter.prevent="send" type="text" placeholder="Parle à l'IA..."
                class="input flex-1 min-h-10" :disabled="isRolling" />

            <!-- Dice Roll / Send button -->
            <div class="flex gap-2 flex-shrink-0">
                <DiceRoll :pending-instruction="pendingInstruction" expr="1d20" @send="send" @rolled="onRolled" />
            </div>
        </div>

        <!-- Status indicators (compact) -->
        <div v-if="isThinking || isRolling" class="mt-2 flex items-center gap-3 text-xs">
            <div v-if="isThinking" class="text-slate-400 flex items-center gap-1">
                <span class="animate-pulse">⏳</span>
                <span>Réflexion en cours...</span>
            </div>
            <div v-if="isRolling && pendingInstruction?.roll"
