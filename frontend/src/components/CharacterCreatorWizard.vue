    ...
    <div class="p-4 rounded-md max-w-4xl mx-auto">
        ...
        <div v-if="currentStep === 0" class="space-y-4">
            ...
            <div>
                ...
                <input v-model="character.name" class="input w-full" placeholder="Ex: Aragorn" />
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div v-if="props.world">
                    ...
                    <div class="text-slate-300 py-2 px-3 bg-slate-800 rounded">{{ props.world }}</div>
                </div>
            </div>

            <div class="flex justify-end gap-2 mt-6">
                <button class="btn-secondary" @click="cancel">Annuler</button>
                <button class="btn-primary" @click="nextStep" :disabled="!character.name">Suivant</button>
            </div>
        </div>

        <!-- Step 2: Race & Class -->
        <div v-if="currentStep === 1" class="space-y-4">
            <h2 class="text-xl font-bold">Race et Classe</h2>
            ...
            <div class="flex justify-end gap-2 mt-6">
                ...
                <button class="btn-primary" @click="nextStep"
                    :disabled="!character.race || !primaryClass">Suivant</button>
            </div>
        </div>

        <!-- Step 3: Ability Scores -->
        <div v-if="currentStep === 2" class="space-y-4">
            <h2 class="text-xl font-bold">Scores de capacités</h2>
            <p class="text-slate-400 text-sm">Répartissez vos scores parmi les 6 capacités</p>

            <AbilityScorePicker v-model:scores="baseScores" :proficiency="2" />
        ...
        <div v-if="currentStep === 3" class="space-y-4">
            ...
            <p class="text-slate-400 text-sm">Sélectionnez {{ skillsToChoose }} compétences pour votre {{ primaryClass
                }}</p>
            ...
            <div class="text-sm text-slate-400">Sélectionnés: {{ selectedSkills.length }}/{{ skillsToChoose }}</div>

            <div class="flex justify-end gap-2 mt-6">
                <button class="btn-secondary" @click="previousStep">Retour</button>
                <button class="btn-primary" @click="finishCreation"
                    :disabled="selectedSkills.length !== skillsToChoose">Créer</button>
            </div>
        </div>

        <!-- Character Preview -->
        <div class="mt-8 p-4 bg-slate-900 rounded border border-slate-700">
            <h3 class="font-bold mb-2">Aperçu du personnage</h3>
            <div class="text-sm text-slate-300 space-y-1">
                ...
                <div v-if="currentStep >= 2"><strong>Scores:</strong> STR {{ baseScores.Str }} | DEX {{ baseScores.Dex
                    }} | CON {{ baseScores.Con }} | INT {{ baseScores.Int }} | WIS {{ baseScores.Wis }} | CHA {{
                    baseScores.Cha }}</div>
                <div v-if="currentStep >= 3"><strong>Compétences:</strong> {{ selectedSkills.join(', ') || '—' }}</div>
            </div>
        </div>
    </div>
</template>
