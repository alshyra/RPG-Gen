<template>
  <div class="worlds">
    ...
    <div class="space-y-3 mt-2">
      <div v-for="w in worlds" :key="w.id" class="tpl flex items-center justify-between p-3 rounded-md bg-slate-800/50 hover:bg-slate-800/70">
        <div class="flex items-center gap-3">
          <div class="logo flex items-center justify-center w-12 h-12 rounded-md text-white font-bold" :class="w.bgClass">
            <!-- simple initials as logo -->
            {{ w.logo }}
          </div>
          <div>
            <div class="text-sm font-semibold text-purple-300">{{ w.name }}</div>
            <div class="text-xs text-slate-400">{{ w.desc }}</div>
          </div>
        </div>

        <div>
          <button class="btn-ghost flex items-center gap-2" @click="$emit('select', w.id)">
            <span>Commencer</span>
