...
import type { CharacterDto, GenderDto } from '@backend/src/character/dto/character.dto';
...
export const useCharacterCreation = () => {
  ...
  const createCharacter = async (characterData: Partial<CharacterDto>) => await store.createCharacter(characterData);
  const updateCharacter = async (characterData: Partial<CharacterDto>) => await store.updateCharacter(characterData);
  const getCharacterById = async (characterId: string) => await store.fetchCharacterById(characterId);
