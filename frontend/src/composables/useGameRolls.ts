...
const getCriticalNote = (diceValue: number): string => (diceValue === 20 ? ' (CRITICAL SUCCESS - Natural 20!)' : diceValue === 1 ? ' (CRITICAL FAILURE - Natural 1!)' : '');

const buildRollMessage = (diceValue: number, bonus: number, skillName: string, total: number, criticalNote: string): string =>
...
export function useGameRolls() {
  ...
  const rollData = ref<RollModalData>({ rolls: [], total: 0, bonus: 0, diceNotation: '', skillName: '' });
  ...
  const onDiceRolled = async (rollResult: any): Promise<void> => {
    ...
    rollData.value = { skillName, rolls: [diceValue], bonus: skillBonus, total: diceValue + skillBonus, diceNotation: instr.dices };
  ...
  const sendRollResult = async (rollResult: any, skillName: string, criticalNote: string): Promise<void> => {
  ...
  const confirmRoll = async (): Promise<void> => {
    ...
    gameStore.appendMessage('System', buildRollMessage(diceValue, bonus, skillName, total, criticalNote));
    try {
