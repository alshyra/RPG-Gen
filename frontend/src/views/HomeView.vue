<template>
  <div class="p-6 max-w-4xl mx-auto">
    <section class="mb-8 text-center">
      <h1 class="text-3xl font-extrabold mb-2">RPG Gemini</h1>
      <p class="text-slate-300">Un moteur d'aventure assisté par Gemini — génère scènes, PNJ, et images. Choisis ton
        univers et commence immédiatement.</p>

      <!-- Resume current character if present -->
      <div v-if="currentCharacter" class="mt-4 mb-6 p-4 bg-slate-800/50 rounded-lg flex items-center justify-between">
        <div class="text-left">
          <div class="text-sm text-slate-300">Personnage en cours</div>
          <div class="text-lg font-semibold">{{ currentCharacter.name || 'Personnage inconnu' }}</div>
          <div class="text-xs text-slate-400">{{ charSummary }}</div>
        </div>
        <div class="flex gap-2">
          <button class="btn-primary" @click="resumeCharacter">Reprendre</button>
          <button class="btn-ghost" @click="clearCurrentCharacter">Supprimer</button>
        </div>
      </div>
...
function onSelect(id: string) {
  try { sessionStorage.setItem('selected-world', id); } catch (e) { }
